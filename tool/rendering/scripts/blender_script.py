...
parser.add_argument("--output_dir", type=str, default="/mnt/chenjh/real2char/mesh2img/test0321/views_whole_sphere")
parser.add_argument(
...
#         bpy.ops.import_scene.obj(filepath=object_path, axis_forward='-Z', axis_up='Y')
#     else:
#         raise ValueError(f"Unsupported file type: {object_path}")
...
# 球状螺旋路径，100个视角，from mingze sun， 2024-03-24
# import numpy as np
...
#     height = np.linspace(-radius, radius, num_cameras)
...
#     empty = bpy.data.objects.new("Empty", None)
#     scene.collection.objects.link(empty)
#     empty.location = (0, 0, 0)  # Make sure empty object is at the origin

#     cam = bpy.data.objects['Camera']
#     cam.constraints.clear()  # Remove any previous constraints

#     num_cameras = args.num_images
#     radius = args.camera_dist  # Use camera_dist as the radius for the spiral
#     num_spirals = 4  # You can adjust this number based on your needs

#     for i in range(num_cameras):
#         set_spherical_spiral_camera(cam, i, num_cameras, radius, num_spirals)
#         render_path = os.path.join(args.output_dir, object_uid, f"{i:03d}.png")
#         bpy.context.scene.render.filepath = render_path
#         bpy.ops.render.render(write_still=True)
#         # Optionally save camera RT matrix as well, if needed
...
# 保持generate_spherical_spiral_camera_views函数不变
def generate_spherical_spiral_camera_views(num_cameras, radius, num_spirals):
    spirals = np.linspace(0, num_spirals * 2 * np.pi, num_cameras)
    height = np.linspace(-radius, radius, num_cameras)
    radius_at_spiral = np.sqrt(radius**2 - height**2)
...
# 修改set_spherical_spiral_camera函数
def set_spherical_spiral_camera(camera, index, num_cameras, radius, num_spirals):
    x, y, z = generate_spherical_spiral_camera_views(num_cameras, radius, num_spirals)
    camera.location = (x[index], y[index], z[index])
    camera.rotation_mode = 'XYZ'
    ...
    camera_matrix = {
        "origin": c2w_matrix.translation[:],  # 使用正确的属性获取位置
        "x": c2w_matrix.col[0][:3],  # 获取正确的列并转化为列表
...
def save_images(object_file: str, args):
    ...
    # 设置渲染分辨率
    bpy.context.scene.render.resolution_x = 1024  # 设置宽度
    bpy.context.scene.render.resolution_y = 1024  # 设置高度
    bpy.context.scene.render.resolution_percentage = 100  # 使用100%的分辨率
    ...
    bg.inputs[0].default_value = (1, 1, 1, 0)  # RGBA, 设置为白色
    ...
    normalize_scene()  # Assuming this adjusts scale and location
    object_uid = os.path.basename(object_file).split(".")[0]
    ...
    for i in range(num_cameras):
        camera_matrix = set_spherical_spiral_camera(cam_ob, i, num_cameras, radius, num_spirals)
        render_path = os.path.join(args.output_dir, object_uid, f"{i:03d}.png")
        bpy.context.scene.render.filepath = render_path
        bpy.ops.render.render(write_still=True)
